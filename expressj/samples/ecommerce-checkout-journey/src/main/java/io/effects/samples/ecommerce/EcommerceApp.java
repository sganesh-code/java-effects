...
import io.effects.recipes.fulfillable.FulfillmentEvent;
import io.effects.recipes.negotiable.NegotiationEvent;
import io.effects.recipes.payable.PaymentEvent;
import io.effects.recipes.reservable.ReservationEvent;
...
public class EcommerceApp {
    ...
    public static void runHappyPathSimulation() {
        ...
        InMemoryEventSubscriber<Object> broker = new InMemoryEventSubscriber<>();

        // Aligned type-safe publishers that delegate to the broker
        EventPublisher<NegotiationEvent<String>> negotiationPublisher = event ->
            broker.publish(event.getClass().getSimpleName(), event);

       EventPublisher<ApprovalEvent<String, String>> approvalPublisher = event ->
            broker.publish(event.getClass().getSimpleName(), event);

       EventPublisher<PaymentEvent<String, Double>> paymentPublisher = event ->
            broker.publish(event.getClass().getSimpleName(), event);

       EventPublisher<ReservationEvent<String, Integer>> reservationPublisher = event ->
            broker.publish(event.getClass().getSimpleName(), event);
            
       EventPublisher<FulfillmentEvent<String, Integer>> fulfillmentPublisher = event ->
            broker.publish(event.getClass().getSimpleName(), event);
