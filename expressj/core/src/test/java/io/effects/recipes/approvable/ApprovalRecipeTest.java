...
class ApprovalRecipeTest {
    ...
    void testExpenseReportAutoApproval() {
        ApprovalProcess<String, String, String> process = new ApprovalProcess<>();
        ...
        Either<String, ApprovalRecord<String, String, String>> submitResult = process.submit("rep-001", "emp-A", "Submitted team lunch", now).unsafeRunSync();
        ...
        List<ApprovalDecision<String, String>> history = record.history();
        assertEquals(1, history.size());
        assertEquals("emp-A", history.get(0).actorId());
        assertNull(history.get(0).actorRole()); // null for initiator
    ...
    void testExpenseReportMidTierManagerApproval() {
        InMemoryStateRepository<String, ApprovalRecord<String, String, String>> repo = new InMemoryStateRepository<>();
        InMemoryEventPublisher<ApprovalEvent<String, String>> publisher = new InMemoryEventPublisher<>();
        ApprovalProcess<String, String, String> process = new ApprovalProcess<>(repo, publisher, new NoOpTelemetryPort());
        ...
        Either<String, ApprovalRecord<String, String, String>> submitResult = process.submit("rep-002", "emp-B", "Submitted Developer Monitor", t0).unsafeRunSync();
        assertTrue(submitResult.isRight());
        ApprovalRecord<String, String, String> record = submitResult.getRight();
        ...
        Either<String, ApprovalRecord<String, String, String>> badApproval = process.approve("rep-002", "vp-X", "VP", "Looks fine to me", t0.plusSeconds(10)).unsafeRunSync();
        ...
        Either<String, ApprovalRecord<String, String, String>> goodApproval = process.approve("rep-002", "mgr-A", "MANAGER", "Approved, within budget", t0.plusSeconds(20)).unsafeRunSync();
        assertTrue(goodApproval.isRight());
        ApprovalRecord<String, String, String> approvedRecord = goodApproval.getRight();

        assertEquals(Status.APPROVED, approvedRecord.status());
        assertNull(approvedRecord.requiredAuthority());

        // Verify published events
        List<ApprovalEvent<String, String>> events = publisher.getPublishedEvents();
    ...
    void testExpenseReportEscalationAndAuditPreservation() {
        ...
        process.submit("rep-003", "emp-C", "Submitted Client Dinner", t0).unsafeRunSync();

        // Escalated by Manager to VP
        Either<String, ApprovalRecord<String, String, String>> escalateResult = process.escalate("rep-003", "mgr-B", "MANAGER", "VP", "Requires higher level validation", t0.plusSeconds(30)).unsafeRunSync();
        assertTrue(escalateResult.isRight());
        ApprovalRecord<String, String, String> escalatedRecord = escalateResult.getRight();

        assertEquals(Status.ESCALATED, escalatedRecord.status());
        assertEquals("VP", escalatedRecord.requiredAuthority());

        // Approved by VP
        Either<String, ApprovalRecord<String, String, String>> approveResult = process.approve("rep-003", "vp-Y", "VP", "VP authorized", t0.plusSeconds(60)).unsafeRunSync();
        assertTrue(approveResult.isRight());
        ApprovalRecord<String, String, String> finalized = approveResult.getRight();

        assertEquals(Status.APPROVED, finalized.status());

        // Verify complete chronology is fully preserved and immutable
        List<ApprovalDecision<String, String>> history = finalized.history();
        assertEquals(3, history.size()); // submit, escalate, approve
        
        assertEquals("emp-C", history.get(0).actorId());
        assertNull(history.get(0).actorRole());
    ...
    void testHealthcareRoutineProcedureFlow() {
        ...
        Either<String, ApprovalRecord<String, String, String>> submitResult = process.submit("med-001", "pat-A", "Submitted medical draw", t0).unsafeRunSync();
        assertTrue(submitResult.isRight());
        assertEquals("CLINICIAN", submitResult.getRight().requiredAuthority());

        // Clinician approves -> Routine, so instantly terminal APPROVED
        Either<String, ApprovalRecord<String, String, String>> approveResult = process.approve("med-001", "doc-A", "CLINICIAN", "Medically necessary", t0.plusSeconds(5)).unsafeRunSync();
        assertTrue(approveResult.isRight());
        ApprovalRecord<String, String, String> finalized = approveResult.getRight();
    ...
    void testHealthcareSurgicalDualApprovalFlow() {
        InMemoryEventPublisher<ApprovalEvent<String, String>> publisher = new InMemoryEventPublisher<>();
        ApprovalProcess<String, String, String> process = new ApprovalProcess<>(new InMemoryStateRepository<>(), publisher, new NoOpTelemetryPort());
        ...
        process.submit("med-002", "pat-B", "Submitted Open Heart Surgery", t0).unsafeRunSync();

        // 1. Clinician approves -> Surgical = true, so transitions to CHIEF_OF_SURGERY required
        Either<String, ApprovalRecord<String, String, String>> clinicianResult = process.approve("med-002", "doc-A", "CLINICIAN", "Surgical triage approved", t0.plusSeconds(5)).unsafeRunSync();
        ...
        Either<String, ApprovalRecord<String, String, String>> prematureInsuranceResult = process.approve("med-002", "ins-rep-A", "INSURANCE_REP", "Pre-authorized", t0.plusSeconds(10)).unsafeRunSync();
        assertTrue(prematureInsuranceResult.isLeft());

        // 2. Chief of Surgery approves -> Transitions to INSURANCE_REP required
        Either<String, ApprovalRecord<String, String, String>> chiefResult = process.approve("med-002", "chief-doc-X", "CHIEF_OF_SURGERY", "OR and surgeon allocated", t0.plusSeconds(20)).unsafeRunSync();
        ...
        Either<String, ApprovalRecord<String, String, String>> insuranceResult = process.approve("med-002", "ins-rep-A", "INSURANCE_REP", "Full coverage authorized", t0.plusSeconds(30)).unsafeRunSync();
        assertTrue(insuranceResult.isRight());
        ApprovalRecord<String, String, String> finalized = insuranceResult.getRight();
    ...
    void testWorkflowInvariantsAndIdempotency() {
        ...
        process.submit("rep-004", "emp-D", "Submitted Office chair", t0).unsafeRunSync();

        // Reject request -> Terminal state REJECTED
        Either<String, ApprovalRecord<String, String, String>> rejectResult = process.reject("rep-004", "mgr-C", "MANAGER", "Chair too expensive", t0.plusSeconds(10)).unsafeRunSync();
        assertTrue(rejectResult.isRight());
        ApprovalRecord<String, String, String> record = rejectResult.getRight();
        assertEquals(Status.REJECTED, record.status());

        // Invariant: Trying to approve a rejected request is prohibited
        Either<String, ApprovalRecord<String, String, String>> tryApprove = process.approve("rep-004", "mgr-C", "MANAGER", "Let me change my mind", t0.plusSeconds(20)).unsafeRunSync();
    ...
    void testTelemetryPortsInjectionSpy() {
        ...
        process.submit("rep-005", "emp-E", "Submitted Notebook", t0).unsafeRunSync();
