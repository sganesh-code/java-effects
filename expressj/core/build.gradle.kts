...
signing {
    ...
    if (!signingKey.isNullOrEmpty()) {
        // Automatically decode Base64 if the key does not start with standard PGP header
        if (!signingKey.trim().startsWith("-----BEGIN PGP")) {
            try {
                signingKey = String(Base64.getDecoder().decode(signingKey.trim()))
            } catch (e: Exception) {
